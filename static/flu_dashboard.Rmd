...
recoded_data <- filtered_results %>%
  mutate_at(vars(-ended), as.character) %>%
...
recoded_data <- recoded_data %>%
  mutate(zipcode = map(zipcode, ~zipfix(.x))) %>%
  mutate(ended = as.Date(ended, format = "%m/%d/%y")) %>%
  mutate_at(vars(-ended), as.character) %>%
  rename(
    "Survey date" = "ended",
...
recoded_data[is.na(recoded_data)] <- 0

cor_input <- recoded_data %>%
  select(-"Survey date", -Zipcode, ) %>%
  mutate_all(as.factor) %>%
  mutate_all(as.numeric) %>%
  cor(method = "spearman")

cor_input[is.na(cor_input)] <- 0
